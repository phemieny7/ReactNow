...
const AnimatedCircle = Animated.createAnimatedComponent(Circle,{useNativeDriver: true});
const AnimatedPath = Animated.createAnimatedComponent(Path,{useNativeDriver: true});
...
export default class TabBar extends Component{
    ...
    update(index) {
        ...
        if (index === 0){
            Animated.spring( that.state.pathD, { toValue: 22,duration: 10, friction: 10,useNativeDriver: true, }).start();
            setTimeout(function() {
                that.setState({
                    showIcon: true,
                });
            }, 100);
            Animated.spring( that.state.circleRadius, { toValue: 211, friction: 10, useNativeDriver: true, } ).start();
        } else if (index === 2){
            Animated.spring( that.state.pathD, { toValue: 691,duration: 10, friction: 10, useNativeDriver: true, }).start();
            ...
            Animated.spring( that.state.circleRadius, { toValue: 880, friction: 10, useNativeDriver: true, } ).start();
        } else {
